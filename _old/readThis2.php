...
if (PHP_SAPI == 'cli')
	...
	for($i=0; $i<sizeof($data);$i++){
		...
		echo $var["id"].' - '.$var["lat"].' - '.$var["lng"].' - '.$var["wifi"].' - '.$var["datetime"].' - '.$var["activity"].' - '.$var["lastcall"].' - '.$var["lastmessage"].'<br><br>';
...
//third sheet
	...
	foreach(array_keys($dataid) as $paramName) { //για κάθε μέρα επανέλαβε
		...
		for($i=0;$i<sizeof($newid);$i++){ //Τύπωσε τα σύνολα
			$objPHPExcel->getActiveSheet()->setCellValue($alphabet[$col].$line, intval($dataid[$paramName][$newid[$i]]));
			//$objPHPExcel->getActiveSheet()->setCellValue('B'.$line, intval($dataid[$paramName][$newid[$i]]));
			//$dataid[$widget][$widgetid][$myhour]=$dataid[$widget][$widgetid][$myhour]+1;
			//for($k=0;$k<24;$k++){
			//	$hourVal = intval($dataid[$widget][$widgetid][$k]);
		...
		for($u=0;$u<24;$u++){
			...
			for($m=0;$m<sizeof($newid);$m++){
				...
				$col=intval($col+$m);
				//$objPHPExcel->getActiveSheet()->setCellValue($alphabet[$col].$line, '"'.$hourVal.'"');
				//$objPHPExcel->getActiveSheet()->setCellValue($alphabet[$col].$line, "A");
				$objPHPExcel->getActiveSheet()->setCellValue("C".$line, $hourVal);
				//$line++;
			}
		$line++;
		}
