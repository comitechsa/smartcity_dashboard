...
if (PHP_SAPI == 'cli')
	...
	$url="https://bd4qol.ddns.net/testing/yesterday";
	$ch = curl_init( $url );
...
//http://drama.smartiscity.gr/readThis2.php
		for($i=0; $i<sizeof($data);$i++){
			...
			if(intval($var['id'])>0){
				...
				if(sizeof($dataSteps[$widget][$widgetid][$myhour])==0){
					$dataSteps[$widget][$widgetid][$myhour]=$var['steps'];
				} else if(intval($var['steps'])<$dataSteps[$widget][$widgetid][$myhour]){
					$dataSteps[$widget][$widgetid][$myhour]=$var['steps'];
				}
			}
...
//third sheet
	...
	foreach(array_keys($dataid) as $paramName) { //για κάθε μέρα επανέλαβε
		...
		for($u=0;$u<24;$u++){ //για όλες τις ώρες ενός 24ωρου επανέλαβε
			...
			/*
			$line++;
			for($z=0;$z<sizeof($newid);$z++){
				$hourVal22 = intval($dataSteps["$paramName"][$newid[$z]][$u]);
				
				$col22=intval($z+2);
				$objPHPExcel->getActiveSheet()->setCellValue($alphabet[$col22].$line, $hourVal22);
				//$line++;
			}			
			*/
...
//third sheet
	...
	$objPHPExcel->getActiveSheet()->setTitle('Steps');
	$idscomma = implode(',',$ids);
...
header('Content-Disposition: attachment;filename="BD4QOL.xlsx"');
