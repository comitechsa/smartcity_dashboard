...
$nav =access;	
$BaseUrl = "index.php?com=passwords-folder";										
...
{	
	...
	if(isset($_POST['recid']) && intval($_POST['recid'])>0) {
		...
		if($db->sql_numrows($result) >= 100 || $_POST['qty']>100) {
			...
			Redirect("index.php?com=passwords-folder");
		}
...
?>
    ...
    if(isset($_GET["item"]) && intval($_GET["item"])>0 && $_REQUEST['Command']=='edit')
    {
        ...
        <div class="row-fluid">
			...
			<a href="index.php?com=passwords-folder"><button type="button" class="btn btn-primary"><?=pageReturn?></button></a>
		</div> <?
	...
	} else if(isset($_GET["date_insert"]))
	{ ?>
		<div class="row-fluid">
			<div class="span12">
				<div class="box box-color box-bordered">
					...
					<div class="box-content nopadding">
						<table class="table table-hover table-nomargin dataTable dataTable-columnfilter table-bordered">
							...
							<tbody>
								<?
									$result = $db->sql_query("SELECT * FROM passwords WHERE date_insert='".$_GET['date_insert']."' AND (user_id=".($auth->UserRow['parent']==0?$auth->UserRow['user_id']:$auth->UserRow['parent'])." OR user_id IN(SELECT user_id FROM users WHERE parent=".($auth->UserRow['parent']==0?$auth->UserRow['user_id']:$auth->UserRow['parent'])."))");	
											...
											while ($dr = $db->sql_fetchrow($result))
											{
												?>
												 <tr>
													...
													<td>
														<a style="padding:4px"  href="index.php?com=passwords-folder&Command=edit&item=<?=$dr["id"]?>"><i class="icon-edit"></i> <?=edit?> </a>
														...
														<a href="#" onclick="ConfirmDelete('<?=deleteConfirm?>','index.php?com=passwords-folder&Command=DELETE&item=<?=$dr["id"]?>');"><span><i class="icon-trash"></i> <?=delete?> </a></span></a>
													</td>
												</tr>
												<?
											}
				...
				<div class="box box-color box-bordered">
					...
					<div class="box-content nopadding">
						<table class="table table-hover table-nomargin dataTable dataTable-columnfilter table-bordered">
							...
							<tbody>
								<?
									$result = $db->sql_query("SELECT distinct date_insert,period FROM passwords WHERE (user_id=".($auth->UserRow['parent']==0?$auth->UserRow['user_id']:$auth->UserRow['parent'])." OR user_id IN(SELECT user_id FROM users WHERE parent=".($auth->UserRow['parent']==0?$auth->UserRow['user_id']:$auth->UserRow['parent']).")) ORDER BY date_insert DESC");										
									if($db->sql_numrows($result) > 0)
									{
										while ($dr = $db->sql_fetchrow($result))
										{ ?>
											 <tr>
												...
												<td><?=$dr["period"]?></td>
												<td>
													<a style="padding:4px"  href="index.php?com=passwords-folder&date_insert=<?=$dr["date_insert"]?>"><i class="icon-edit"></i> <?=edit?> </a>
													<a style="padding:4px"  target="_blank" href='http://panel.spotyy.com/sites/hotbox/components/hotbox/passwords-pdf.php?date_insert=<?=$dr["date_insert"]?>&id=<?=$auth->UserId?>&recID=<?=$dr["id"]?>'><i class="icon-print"></i> <?=print2laser?> </a>
													<a style="padding:4px"  target="_blank" href='http://panel.spotyy.com/sites/hotbox/components/hotbox/tcpdf/passwords-pdf-thermal.php?date_insert=<?=$dr["date_insert"]?>&id=<?=$auth->UserId?>&recID=<?=$dr["id"]?>'><i class="icon-print"></i> <?=print2thermal?> </a>
												</td>
											</tr>
												<?
										}
