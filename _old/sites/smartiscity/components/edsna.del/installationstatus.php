...
global $nav;
$nav = 'Καταστάσεις εγκαταστάσεων';
$config["navigation"] = $nav;
$item = (intval($_GET["item"]) > 0 ? intval($_GET["item"]) : "");
$BaseUrl = "index.php?com=installationstatus";
$command=array();
...
{	
	if($_REQUEST["Command"] == "SAVE")
	{
		...
		{
			$PrimaryKeys["installationstatus_id"] = intval($_GET["item"]);
		...
		$db->ExecuteUpdater("installationstatus",$PrimaryKeys,$Collector,$QuotFields);		
		$messages->addMessage("SAVED!!!");
		Redirect($BaseUrl);
	} else if($_REQUEST["Command"] ==  "DELETE") { 
		if($item != ""){
			//SOS Έλεγχο πριν τη διαγραφή
			$db->sql_query("DELETE FROM installationstatus WHERE municipality_id=" . $item);
...
if(isset($_GET["item"])) {
	$query="SELECT * FROM installationstatus WHERE installationstatus_id=".$_GET['item'];
	$dr_e = $db->RowSelectorQuery($query);
	if (!isset($dr_e["installationstatus_id"]) && intval($_GET['item'])>0) {
		$messages->addMessage("NOT FOUND!!!");
		Redirect("index.php?com=installationstatus");
	}
	?>
	...
	<div class="row-fluid"> 
		<div class="span12">            
			...
			<div class="box-content nopadding">
				...
				<div class="control-group">
					<div class="controls">
						<label for="discount" class="control-label">Έκπτωση</label>
						<input type="text" name="discount" id="discount" name class="input-xxlarge valid"<?=(isset($dr_e["discount"]) ? 'value="'.$dr_e['discount'].'"' : "")?> >
					</div>
				</div>
				<div class="control-group">
			...
			<a href="index.php?com=installationstatus"><button type="button" class="btn btn-primary">Επιστροφή</button></a>
    </div>
		...
		<div class="span12">
			...
			<div class="box-content nopadding">
				<table class="table table-hover table-nomargin dataTable dataTable-columnfilter table-bordered">
					...
					<tbody> 
					<?	
						$query = "SELECT * FROM installationstatus WHERE 1=1 ".$filter." ORDER BY name ";
                        ...
                        while ($dr = $db->sql_fetchrow($result))
                        {
                            ?>
                                <tr>
									<td><?=$dr["installationstatus_id"]?></td>
                                    <td><?=$dr["name"]?></td>
									<td><?=$dr["discount"]?></td>
                                    <td>
                                        <a style="padding:4px"  href="index.php?com=installationstatus&Command=edit&item=<?=$dr["installationstatus_id"]?>"><i class="icon-edit"></i> <?=edit?></a>
                                        <a href="#" onclick="ConfirmDelete('<?=deleteConfirm?>','index.php?com=installationstatus&Command=DELETE&item=<?=$dr["installationstatus_id"]?>');"><span><i class="icon-trash"></i><?=delete?></a></span></a>
                                    </td>
                                </tr>
                            <?
                        }
