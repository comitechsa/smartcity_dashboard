					...
					<table class="table table-hover table-nomargin dataTable dataTable-columnfilter table-bordered">
						...
						<tbody>
							<?
								...
								$query = "SELECT t1.*,t2.name FROM vehicles t1 INNER JOIN municipalities t2 ON t1.municipality_id=t2.municipality_id WHERE 1=1  " . $filter . " ORDER BY plate ";
								...
								<tr>
									<td><?= $dr["vehicle_id"] ?></td>
									<td><?= $dr["is_valid"] ?></td>
									<? if($auth->UserType == "Administrator") echo '<td>'.$dr["name"] .'</td>';?>
									<td><?= $dr["plate"] ?></td>
									<td><?= $dr["imei"] ?></td>
