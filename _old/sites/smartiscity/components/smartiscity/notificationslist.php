...
{	
	if($_REQUEST["Command"] == "SAVE")
	{
		...
		if($item != ""){
			...
			$db->sql_query("DELETE FROM notifications WHERE notification_id=" . $item);
...
if(isset($_GET["item"])) {
	$query="SELECT * FROM notifications WHERE notification_id=".$_GET['item'];
				...
				<div id="add-listing" class="separated-form">
					...
					<div class="add-listing-section">
						...
						<div class="add-listing-headline">
							<h3 style="font-family: 'Commissioner', sans-serif;"><i class="sl sl-icon-doc"></i> Ειδοποιήσεις</h3>
						</div>
...
<script>
	...
	<div class="container" style="margin-bottom:50px;font-size:14px;">
						...
						<?	$filter='';
							...
							while ($dr = $db->sql_fetchrow($result))
							{
								?>
									...
									<tr>
										<td><?=$dr["notification_id"]?></td>
										<td><?=$dr["is_valid"]?></td>
										<td><?=$dr["title"]?></td>
										<td><?=$dr["date_insert"]?></td>
										<td>
											<a style="padding:4px"  href="index.php?com=notificationslist&Command=edit&item=<?=$dr["notification_id"]?>"><span style="font-size:22px;" class="im im-icon-File-Edit"></span> </a>
											<a href="#" onclick="ConfirmDelete('<?=deleteConfirm?>','index.php?com=notificationslist&Command=DELETE&item=<?=$dr["notification_id"]?>');"><span  style="font-size:22px;" class="im im-icon-Delete-File"></span></a>
										</td>
									</tr>
								<?
							}
		...
		<div class="row" style="margin-top:30px;">
			<div class="col-md-12">
				<a href="index.php?com=notificationslist&item=" class="button border">Εισαγωγή</a><a href="index.php" class="button border">Επιστροφή</a>
			</div>
		</div>
	</div>
<? } ?> 
